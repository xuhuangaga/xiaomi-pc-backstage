...
export default {
  ...
  // 获取订单
  getOrder({ current, pageSize, query }: { current: number, pageSize: number, query: string }) {
    return http.get(`/admin/getOrder?current=${current}&pageSize=${pageSize}&query=${query}`)
  },
  ...
  // 获取商品规格
  getSpec({ parentId,current,pageSize }: { parentId: string,current:number,pageSize:number }) {
  ...
  //修改商品分类状态
  showCategory({ id, isShow }: { id: string, isShow: boolean }) {
    return http.post('/admin/showCategory', {
      id,
      isShow
    })
  },
  ...
  // 获取
  getParams({ parentId,current,pageSize }: { parentId: string,current:number,pageSize:number }) {
  ...
  // 修改秒杀
  updateSeckill({ start_time, end_time, goods_number, goods, isShow,id }: { id:string,start_time: string, end_time: string, goods_number: string, goods: string, isShow: boolean }) {
